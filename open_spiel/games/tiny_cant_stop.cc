...
int NumCheckersPerPlayer(const Game* game) {
  return static_cast<const TinyCantStopGame*>(game)->NumCheckersPerPlayer();
}
...
int TinyCantStopState::AugmentCheckerMove(CheckerMove* cmove, int player,
                                        int start) const {
  int end = cmove->num;
  if (end != kPassPos) {
...
std::string TinyCantStopState::ActionToString(Player player,
...
TinyCantStopState::TinyCantStopState(std::shared_ptr<const Game> game,
...
void TinyCantStopState::SetupInitialBoard() {
...
int TinyCantStopState::board(int player, int pos) const {
  if (pos == kBarPos) {
    return bar_[player];
  } else {
...
Player TinyCantStopState::CurrentPlayer() const {
  return IsTerminal() ? kTerminalPlayerId : Player{cur_player_};
}

int TinyCantStopState::Opponent(int player) const { return 1 - player; }

void TinyCantStopState::RollDice(int outcome) {
  dice_.push_back(kChanceOutcomeValues[outcome][0]);
  dice_.push_back(kChanceOutcomeValues[outcome][1]);
}

int TinyCantStopState::DiceValue(int i) const {
  SPIEL_CHECK_GE(i, 0);
...
void TinyCantStopState::UndoAction(int player, Action action) {
  {
...
bool TinyCantStopState::IsHit(Player player, int from_pos, int num) const {
...
Action TinyCantStopState::TranslateAction(int from1, int from2,
...
Action TinyCantStopState::EncodedBarMove() const { return 24; }

Action TinyCantStopState::EncodedPassMove() const { return 25; }

Action TinyCantStopState::CheckerMovesToSpielMove(
...
std::vector<CheckerMove> TinyCantStopState::SpielMoveToCheckerMoves(
...
std::vector<CheckerMove> TinyCantStopState::AugmentWithHitInfo(
...
bool TinyCantStopState::IsPosInHome(int player, int pos) const {
  switch (player) {
    case kXPlayerId:
...
int TinyCantStopState::CheckersInHome(int player) const {
  int c = 0;
  for (int i = 0; i < 6; i++) {
...
bool TinyCantStopState::AllInHome(int player) const {
  if (bar_[player] > 0) {
    return false;
  }
...
int TinyCantStopState::HighestUsableDiceOutcome() const {
...
int TinyCantStopState::FurthestCheckerInHome(int player) const {
...
bool TinyCantStopState::UsableDiceOutcome(int outcome) const {
  return (outcome >= 1 && outcome <= 6);
}

int TinyCantStopState::PositionFromBar(int player, int spaces) const {
  if (player == kXPlayerId) {
    return -1 + spaces;
  } else if (player == kOPlayerId) {
    return 24 - spaces;
...
int TinyCantStopState::PositionFrom(int player, int pos, int spaces) const {
  if (pos == kBarPos) {
...
int TinyCantStopState::NumOppCheckers(int player, int pos) const {
  return board_[Opponent(player)][pos];
}

int TinyCantStopState::GetDistance(int player, int from, int to) const {
...
bool TinyCantStopState::IsOff(int player, int pos) const {
  // Returns if an absolute position is off the board.
  return ((player == kXPlayerId && pos > 23) ||
          (player == kOPlayerId && pos < 0));
}

bool TinyCantStopState::IsFurther(int player, int pos1, int pos2) const {
  if (pos1 == pos2) {
    return false;
  }
...
int TinyCantStopState::GetToPos(int player, int from_pos, int pips) const {
  if (player == kXPlayerId) {
...
bool TinyCantStopState::IsLegalFromTo(int player, int from_pos, int to_pos,
...
std::string TinyCantStopState::DiceToString(int outcome) const {
  if (outcome > 6) {
...
int TinyCantStopState::CountTotalCheckers(int player) const {
  int total = 0;
  for (int i = 0; i < 24; ++i) {
...
int TinyCantStopState::IsGammoned(int player) const {
  if (hyper_backgammon_) {
...
int TinyCantStopState::IsBackgammoned(int player) const {
  if (hyper_backgammon_) {
    // TODO(author5): remove this when the doubling cube is implemented.
    // In Hyper-backgammon, gammons and backgammons only multiply when the cube
    // has been offered and accepted. However, we do not yet support the cube.
...
std::set<CheckerMove> TinyCantStopState::LegalCheckerMoves(int player) const {
...
bool TinyCantStopState::ApplyCheckerMove(int player, const CheckerMove& move) {
  // Pass does nothing.
  if (move.pos < 0) {
    return false;
  }
...
void TinyCantStopState::UndoCheckerMove(int player, const CheckerMove& move) {
...
int TinyCantStopState::RecLegalMoves(
...
std::vector<Action> TinyCantStopState::ProcessLegalMoves(
...
std::vector<Action> TinyCantStopState::LegalActions() const {
  ...
  TinyCantStopState* state = dynamic_cast<TinyCantStopState*>(cstate.get());
  std::set<std::vector<CheckerMove>> movelist;
...
void TinyCantStopState::SetState(int cur_player, bool double_turn,
...
double TinyCantStopGame::MaxUtility() const {
  if (hyper_backgammon_) {
...
int TinyCantStopGame::NumCheckersPerPlayer() const {
  if (hyper_backgammon_) {
    return 3;
  } else {
