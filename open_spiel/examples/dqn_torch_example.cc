...
void SelfPlayTinyCantStop(int seed, int total_episodes, int report_every,
                       int num_eval_episodes) {
  std::cout << "Running self-play Tiny Can't Stop" << std::endl;
  std::mt19937 rng(seed);
  std::shared_ptr<const Game> game = open_spiel::LoadGame("tiny_cant_stop");
...
int main(int argc, char** argv) {
  ...
  torch_dqn::SelfPlayTinyCantStop(seed,
                               /*total_episodes*/ 2000,
                               /*report_every*/ 10,
