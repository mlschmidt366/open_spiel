...
std::unique_ptr<open_spiel::Bot> InitBot(
  ...
  return std::make_unique<open_spiel::algorithms::StochasticMCTSBot>(
      game, std::move(evaluator), UCT_C, max_simulations,
      /*max_memory_mb=*/5, /*solve=*/true, /*seed=*/42, /*verbose=*/false);
}

void StochasticMCTSTest_CanPlayTicTacToe() {
...
void StochasticMCTSTest_CanPlayBothSides() {
  auto game = LoadGame("tic_tac_toe");
  int max_simulations = 100;
  auto evaluator =
      std::make_shared<open_spiel::algorithms::RandomRolloutEvaluator>(20, 42);
...
void StochasticMCTSTest_CanPlaySinglePlayer() {
...
void StochasticMCTSTest_CanPlayThreePlayerStochasticGames() {
...
SearchTicTacToeState(const absl::string_view initial_actions) {
  ...
  auto evaluator =
      std::make_shared<open_spiel::algorithms::RandomRolloutEvaluator>(20, 42);
  algorithms::StochasticMCTSBot bot(*game, evaluator, UCT_C,
...
void StochasticMCTSTest_SolveDraw() {
...
void StochasticMCTSTest_SolveLoss() {
  auto [root, state] =
...
void StochasticMCTSTest_SolveWin() {
...
void StochasticMCTSTest_GarbageCollect() {
  auto game = LoadGame("tic_tac_toe");
  std::unique_ptr<State> state = game->NewInitialState();
  auto evaluator =
  ...
  SPIEL_CHECK_TRUE(root->outcome.size() == 2 ||
                   root->propagation_count == 1000000);
}
...
int main(int argc, char** argv) {
  open_spiel::StochasticMCTSTest_CanPlayTicTacToe();
  open_spiel::StochasticMCTSTest_CanPlayBothSides();
  open_spiel::StochasticMCTSTest_CanPlaySinglePlayer();
  open_spiel::StochasticMCTSTest_CanPlayThreePlayerStochasticGames();
  open_spiel::StochasticMCTSTest_SolveDraw();
  open_spiel::StochasticMCTSTest_SolveLoss();
  open_spiel::StochasticMCTSTest_SolveWin();
  open_spiel::StochasticMCTSTest_GarbageCollect();
}
