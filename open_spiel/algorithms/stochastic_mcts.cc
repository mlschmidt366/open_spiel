...
#include "open_spiel/algorithms/stochastic_mcts.h"
...
double SearchNode::UCTValue(double parent_explored, double uct_c) const {
  if (!outcome.empty()) {
...
double SearchNode::PUCTValue(double parent_explored, double uct_c) const {
...
bool SearchNode::CompareFinal(const SearchNode& b) const {
  ...
  if (explored != b.explored) {
    return explored < b.explored;
  }
...
std::string SearchNode::ToString(const State& state) const {
  return absl::StrFormat(
      ...
      (action != kInvalidAction ? state.ActionToString(player, action)
                                : "none"),
      player, prior, (explored ? total_reward / explored : 0.),
      propagation_count,
      (outcome.empty()
           ? "none"
           : absl::StrFormat("%4.1f",
...
StochasticMCTSBot::StochasticMCTSBot(const Game& game, std::shared_ptr<Evaluator> evaluator,
...
Action StochasticMCTSBot::Step(const State& state) {
  ...
  if (verbose_) {
    ...
    std::cerr
        << absl::StrFormat(
               ...
               root->propagation_count, seconds, (root->propagation_count / seconds),
...
std::pair<ActionsAndProbs, Action> StochasticMCTSBot::StepWithPolicy(const State& state) {
  Action action = Step(state);
  return {{{action, 1.}}, action};
}

std::unique_ptr<State> StochasticMCTSBot::ApplyTreePolicy(
    SearchNode* root, const State& state,
    std::vector<SearchNode*>* visit_path) {
  visit_path->push_back(root);
  std::unique_ptr<State> working_state = state.Clone();
  SearchNode* current_node = root;
  while (!working_state->IsTerminal() && current_node->propagation_count > 0) {
      ...
      for (SearchNode& child : current_node->children) {
        ...
        switch (child_selection_policy_) {
          case ChildSelectionPolicy::UCT:
            val = child.UCTValue(current_node->explored, uct_c_);
            break;
          case ChildSelectionPolicy::PUCT:
            val = child.PUCTValue(current_node->explored, uct_c_);
            break;
        }
        if (val > max_value) {
          max_value = val;
...
std::unique_ptr<SearchNode> StochasticMCTSBot::MCTSearch(const State& state) {
  ...
  for (int i = 0; i < max_simulations_; ++i) {
    ...
    for (auto it = visit_path.rbegin(); it != visit_path.rend(); ++it) {
      ...
      // TODO: how to change solving the MCTS tree with fractional explore_counts?
...
void StochasticMCTSBot::GarbageCollect(SearchNode* node) {
  if (node->children.empty()) {
    return;
  }
  bool clear_children = node->propagation_count < gc_limit_;
