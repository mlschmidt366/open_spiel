...
#ifndef OPEN_SPIEL_ALGORITHMS_STOCHASTIC_MCTS_H_
#define OPEN_SPIEL_ALGORITHMS_STOCHASTIC_MCTS_H_
...
struct SearchNode {
  ...
  double explored = 0;          // value indicating how much this node was explored in relation to the random events following it.
  int propagation_count = 0;        // Number of times this node was explored.
  ...
  double UCTValue(double parent_explored, double uct_c) const;

  // The value as returned by the PUCT formula.
  double PUCTValue(double parent_explored, double uct_c) const;
...
class StochasticMCTSBot : public Bot {
 public:
  ...
  ~StochasticMCTSBot() = default;

  void Restart() override {}
  void RestartAt(const State& state) override {}
  // Run StochasticMCTS for one step, choosing the action, and printing some information.
  ...
  // Run StochasticMCTS on a given state, and return the resulting search tree.
